...
export default function App() {
  ...
  // Measure container size (not image size!)
  useEffect(() => {
  ...
  // 计算图片在容器中的实际渲染位置（object-contain 模式）
  ...
  const convertHotspotPosition = (spot: Hotspot) => {
    console.log('');
    console.log('========== 🔍 热点位置计算 [' + spot.id + '] ==========');
    
    // 获取原始坐标值
    const rawX = spot.rawX ?? (parseFloat(String(spot.x)) || 0);
    const rawY = spot.rawY ?? (parseFloat(String(spot.y)) || 0);
    console.log('📍 热点原始坐标: rawX =', rawX, ', rawY =', rawY);
    
    // 使用 AI 返回的图片尺寸来计算百分比（如果有的话）
    // 这是关键修复：AI 返回的坐标是基于它看到的图片尺寸
    const imgW = (spot as any).aiImageWidth || imageWidth || 2048;
    const imgH = (spot as any).aiImageHeight || imageHeight || 1280;
    console.log('📐 图片尺寸(AI返回):', imgW, 'x', imgH);
    console.log('📐 containerRect:', containerRect ? `${containerRect.width.toFixed(0)} x ${containerRect.height.toFixed(0)}` : 'null (未获取)');
    
    // 智能判断坐标类型：
    // - 如果坐标值 <= 100，认为是百分比值，直接使用
    // - 如果坐标值 > 100，认为是像素值，需要转换为百分比
    ...
    if (rawX <= 100 && rawY <= 100) {
      ...
      percentX = rawX;
      percentY = rawY;
      console.log('📊 判断为百分比坐标，直接使用');
    } else {
      // 像素坐标，转换为百分比（使用 AI 返回的图片尺寸）
      percentX = Math.min(100, Math.max(0, (rawX / imgW) * 100));
      percentY = Math.min(100, Math.max(0, (rawY / imgH) * 100));
      console.log('📊 判断为像素坐标，用AI尺寸转换:', `${rawX}/${imgW}=${percentX.toFixed(1)}%`, `${rawY}/${imgH}=${percentY.toFixed(1)}%`);
    }
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          console.log('📐 压缩比例:', (width * height / (img.width * img.height) * 100).toFixed(1) + '%');
...
请确保 imageWidth 和 imageHeight 是你实际看到的图片尺寸！`;
      ...
      console.log('========== 🤖 AI 分析结果 ==========');
      console.log('📐 AI返回的图片尺寸:', aiImageWidth, 'x', aiImageHeight);
      console.log('📐 本地压缩后图片尺寸:', imageWidth, 'x', imageHeight);
      console.log('🎯 AI返回的原始热点数据:', JSON.stringify(result.hotspots, null, 2));
      console.log('📦 AI返回的材料数据:', JSON.stringify(result.materials, null, 2));
      
      // 处理热点 - 去重（同一 materialType 只保留一个）
      ...
      if (materials.length !== hotspots.length) {
        materials = hotspots.map((spot, i) => ({
          id: i + 1,
          name: spot.materialType || spot.materialName || `材料${i + 1}`,
          type: spot.materialName || '其他',
          characteristics: '环保,耐用,美观',
          base_price_min: 100,
          base_price_max: 300,
              ...
              <div className="lg:col-span-8 space-y-6">
                <div ref={imageContainerRef} className="relative bg-slate-900 rounded-2xl overflow-hidden border border-slate-800">
                  ...
                  {analysisResult.hotspots && analysisResult.hotspots.length > 0 ? (
                    (() => {
                      console.log('');
                      console.log('========== 🖼️ 渲染热点时的容器信息 ==========');
                      console.log('📐 containerRect:', containerRect ? `${containerRect.width.toFixed(0)} x ${containerRect.height.toFixed(0)}` : 'null');
                      console.log('📐 imageWidth:', imageWidth, ', imageHeight:', imageHeight);
                      console.log('🎯 热点数量:', analysisResult.hotspots.length);
                      return null;
                    })(),
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <p className="text-slate-400 text-sm">未检测到热点位置</p>
                    </div>
                  )}
                  ...
                  <div className="absolute bottom-6 left-6 right-6 bg-slate-900/80 backdrop-blur-md p-4 rounded-xl border border-white/10 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm font-bold">{analysisResult.materials?.length || 0} 项材料</span>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      <button 
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
                  {selectedHotspot ? (
                    <div className="space-y-8">
                      ...
                      {(() => {
                        // Find matching material using materialType (more accurate match)
                        ...
                        <>
                          ...
                          <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                            <p className="text-sm text-amber-400">暂未获取到该材料的详细价格信息</p>
                          </div>
                        );
                      })()}
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <select 
                        ...
                        className="w-full bg-slate-800 border-slate-700 rounded-lg text-xs p-2"
                      >
                        <option value="wanx2.1-t2i-turbo">wanx2.1-t2i-turbo (推荐)</option>
